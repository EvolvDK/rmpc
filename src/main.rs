...
fn main() -> Result<()> {
    ...
    if let Some(Command::ImportLibrary { .. }) | Some(Command::ImportPlaylists { .. }) =
        args.command
    {
        initial_command = args.command.take();
    }
            ...
            if let Some(cmd) = initial_command {
                match cmd {
                    Command::ImportLibrary { path } => {
                        event_tx.send(AppEvent::UiEvent(UiAppEvent::ImportYouTubeLibrary {
                            path,
                        }))?;
                    }
                    Command::ImportPlaylists { path } => {
                        event_tx.send(AppEvent::UiEvent(UiAppEvent::ImportYouTubePlaylists {
                            path,
                        }))?;
                    }
                    _ => {}
